...
export default function StudentProfile() {
  ...
  const [organizationStatus, setOrganizationStatus] = useState('Inactive');
  const [councilStatus, setCouncilStatus] = useState('Inactive');
  ...
  const fetchMembershipStatus = async () => {
    try {
      ...
      // Check organization membership (ELITES, SPECS, IMAGES)
      ...
      if (!councilSnapshot.empty) {
        const subscription = councilSnapshot.docs[0].data();
        const endDate = subscription.endDate?.toDate();
        if (endDate && endDate > new Date()) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <h3 className="text-lg font-bold text-slate-900 mb-4">Membership Status</h3>
          ...
          <div className="p-4 bg-slate-50 rounded-lg">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-slate-900">
                  {profileData.program === 'BSIT' ? 'ELITES' : profileData.program === 'BSCS' ? 'SPECS' : profileData.program === 'BSEMC' ? 'IMAGES' : 'Organization'} Membership
                </span>
              </div>
            ...
            <p className="text-xs text-slate-600">
              {organizationStatus === 'Active' 
                ...
                : 'Subscribe to activate your organization membership'}
            </p>
          </div>
          ...
          <div className="p-4 bg-slate-50 rounded-lg">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-slate-900">Council Membership</span>
              </div>
              <span className={`px-3 py-1 text-xs font-semibold rounded-full ${
            ...
            <p className="text-xs text-slate-600">
              {councilStatus === 'Active' 
                ? 'Your council membership is currently active' 
                : 'Subscribe to activate your council membership'}
            </p>
          </div>
        </div>
      </div>
